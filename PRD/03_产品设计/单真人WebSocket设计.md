...
9. Token：**密码学安全随机会话 Token**，绑定 `room_id` + `seat_id`，仅本局有效；**禁止放 WebSocket URL 查询参数**；首次连接后通过 `join`/`reconnect` 鉴权消息提交；服务端**只保存摘要**，不写日志。
...
**容量与回收（MVP）**：
- `MAX_ACTIVE_ROOMS`（可配置）；达到上限后 `create_room` 返回 `room_capacity_reached`。
...
- `stream_seq`：**每房所有下行消息**统一递增（用于断线补发），与游戏无关。
...
| `joined` | 入座成功（含 token） | `room_id`,`seat_id`,`token` |
...
│ join(token) ─────────▶│ joined(seat=0, token)      │
│ start ───────────────▶│ 开对局线程 ────────────────▶│ run()
...
2. 客户端断线后重连：发 `reconnect {room_id, seat_id, token, last_stream_seq}`。
3. 服务端校验 token 摘要与 seat 归属，把 `stream_seq > last_stream_seq` 且**该座位可见**的消息逐条补发（可见性过滤仍然逐条执行）。
4. 回 `reconnected {latest_stream_seq, replayed_count}`。
5. 60s 宽限期内恢复操作；超期进入**永久自动兜底模式**（可继续观战/终局回放，不可再提交操作）。
...
- `client_action_id`（客户端生成）保证重复提交幂等：同 id 只处理一次，重复 → `duplicate_action`。
...
- 单条消息大小上限（如 16KB）、发送频率上限（令牌桶），超限回 `message_too_large` / `rate_limited`。
...
- `MAX_ACTIVE_ROOMS` 与单条消息大小/频率上限的具体数值（可给默认值，实现前定）。
- 断线宽限 60s 与各操作超时的默认值是否进入配置文件（建议是）。
