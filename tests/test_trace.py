...
def test_decision_record_carries_three_channels_and_delta():
    ...
    assert record.key_player is not None
...
def test_unauthorized_evidence_is_rejected():
    text = _json(evidence="seer_result")
    bot = LLMBot(0, _FixedProvider(text), PERSONAS["nice"])
    bot.decide(_view(Role.VILLAGER), _vote_request())
    record = bot.latest_record
    assert record is not None
    assert record.fallback_reason is not None
    assert "unauthorized evidence" in record.fallback_reason
...
def test_record_to_dict_round_trips():
    record = DecisionRecord(
        ...
        evidence="pack", candidates=(0, 1), decision="vote P0",
    ...
    assert reloaded["deception"] is True
    assert reloaded["decision"] == "vote P0"
    assert reloaded["private_suspicion"]["0"] == 0.0  # JSON keys become strings
